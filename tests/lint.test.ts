...
test("an array of embedded documents with no expected size is reported", () => {
  ...
  assert.match(en(finding!.message), /16MB/);
  assert.match(en(finding!.message), /@count\(n\)/, "the finding says what to do about it");
});
...
test("a big array of references suggests turning the link around", () => {
  ...
  assert.match(en(finding!.message), /Storing the link on 'post' instead/);
});
...
test("5000 small embedded documents is a quarter of a megabyte, and not yet a problem", () => {
  ...
  assert.ok(bytes > 200_000 && bytes < 400_000, en(formatBytes(bytes)));
...
test("nesting past the limit is reported exactly once", () => {
  ...
  assert.match(en(found[0]!.message), /5 levels deep/);
...
test("a document over the limit says so plainly", () => {
  ...
  assert.equal(finding!.level, "warning");
  assert.match(en(finding!.message), /cannot be written as one document/);
...
test("sizes are written for a person to read", () => {
  ...
  assert.equal(en(formatBytes(3 * 1024 * 1024)), "3.0 MB");
});
